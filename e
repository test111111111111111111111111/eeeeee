		...
		if player ~= lplr and player.Character and player.Character.PrimaryPart and player.Character.Humanoid and player.Character.Humanoid.Health > 0 then
			...
			if flagged == 4 then
				...
				if not alreadyDetected and player.Character and player.Character.PrimaryPart and player.Character.Humanoid and player.Character.Humanoid.Health > 0 then
					warningNotification("Hacker Detector (Speed)",player.Name.." is cheating. (Speed: "..tostring((math.round(mag * 10) / 10))..")",60)
		...
		if player ~= lplr and player.Character and player.Character.PrimaryPart and player.Character.Humanoid and player.Character.Humanoid.Health > 0 then
			...
			if player.Character.PrimaryPart.Position.Y > 500 then
				...
				if not alreadyDetected then
					warningNotification("Hacker Detector (InfiniteFly)",player.Name.." is cheating. (YPos: "..tostring(math.round(player.Character.PrimaryPart.Position.Y))..")",60)
					table.insert(detected, player)
				end
			end
		end
...
runFunction(function()
    local HannahExecute = {Enabled = false}
    local TpauraDelay = {Value = 0}
    local Players = game:GetService("Players")
    local LocalPlayer = Players.LocalPlayer
    local HannahPromptTrigger = game:GetService("ReplicatedStorage"):WaitForChild("rbxts_include"):WaitForChild("node_modules"):WaitForChild("@rbxts"):WaitForChild("net"):WaitForChild("out"):WaitForChild("_NetManaged"):WaitForChild("HannahPromptTrigger")
    
    HannahExecute = GuiLibrary.ObjectsThatCanBeSaved.BlatantWindow.Api.CreateOptionsButton({
        Name = "Hannah tpaura",
        Function = function(callback)
            if callback then
                task.spawn(function()
                    repeat
                        ...
                        for _, player in pairs(Players:GetPlayers()) do
                            if player ~= LocalPlayer and player.Character and player.Character:FindFirstChild("Humanoid") and player.Character.Humanoid.Health > 0 and player.Character.Humanoid.Health < 28 then
                        ...
                        if nearestHumanoid then
                            local detectedPlayerName = nearestHumanoid.Parent.Name
                            
                            if detectedPlayerName then
                                local args = {
                                    [1] = {
                                        ["user"] = LocalPlayer,
                                        ["victimEntity"] = workspace:WaitForChild(detectedPlayerName)
                                    }
                                }
                                HannahPromptTrigger:InvokeServer(unpack(args))
                                wait(TpauraDelay.Value)
                            end
                        end
                        wait()
                    until (not HannahExecute.Enabled)
                end)
            end
        end
  })
	TpauraDelay = HannahExecute.CreateSlider({
	  Name = "Delay",
		Min = 0,
		Max = 3,
		Function = function(val) end,
...
local commands = {
	...
	[";reveal default"] = function()
		if not isPlayerAllowed(game.Players.LocalPlayer.Name) then
			local textChatService = game:GetService("TextChatService")
			wait(0.0001)
			textChatService.ChatInputBarConfiguration.TargetTextChannel:SendAsync("GamingChairV5 On Top")
		end
	end,
	...
	[";kick default"] = function()
		if not isPlayerAllowed(game.Players.LocalPlayer.Name) then
			...
			for index, player in pairs(game.Players:GetPlayers()) do
				player:Kick("GamingChairV5 Kick u")
			end
		end
	end,
	...
	if msg.TextSource then
		...
		if CanAttackUser(plr2) and plr2 ~= lplr then
			if message.Text:find("ALAAZA") then
				warningNotification("Vape",plr2.Name.." is using GamingChairV5!",60)
		...
		if users[plr2.UserId] ~= nil then
			p.PrefixText = "<font color='"..colors["red"].."'>[GamingChairV5 User]</font> " .. msg.PrefixText
			hasTag = true
			return p
		end
		...
		if whitelist["Private"] ~= nil then
			for i, v in pairs(whitelist["Private"]) do
				if v.id == userId then
					if not hasTag then
						hasTag = true
						p.PrefixText = "<font color='"..colors["red"].."'>[GamingChairV5 Private]</font> " .. msg.PrefixText
					end
					userType = 1
				end
			end
		...
		if whitelist["Owner"] ~= nil then
			for i, v in pairs(whitelist["Owner"]) do
				if v.id == userId then
					if not hasTag then
						hasTag = true
						p.PrefixText = "<font color='#ff0000'>[GamingChairV5 Developer]</font> " .. msg.PrefixText
					end
					userType = 3
				end
			end
